...
void test_case_cache1(void)
{
    ...
    memset(current, 0, sizeof(cache_buffer)-sizeof(pthread_mutex_t));
...
void test_case_cache3(void)
{
    ...
    unlink("identity.db");
    UID_getLocalIdentity("tprv8ZgxMBicQKsPespLkTajWG7HSrdBv2TgmUr6zuWXeZLy8VjcS7tQ6jpTBn7vhCMEXZ1dNWtbAY3SWACc1GGby325e2rcaHbaRKuADtxSJdW");
    memset(current, 0, sizeof(cache_buffer)-sizeof(pthread_mutex_t));
    CU_ASSERT(0 == UID_getContracts(&cache));
